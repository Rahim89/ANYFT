...
public class SitesDAOImpl extends OracleConnection implements SitesDAO {
	...
	public int addSites(Sites site) {
		int key =0;
		String sql= "INSERT INTO sites(siteName,mainPicture,timeneed) VALUES(?,?,?)";
	...
	public void updateSite(Sites site) {
		...
		String sql = "UPDATE sites SET siteName =?, mainPicture =?, timeneed =? where siteId = ?";
		OracleConnection c = new OracleConnection();
		Connection conn=null;
		try {
			conn = c.getConnection();
